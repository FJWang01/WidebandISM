...
im_Or_d_mat = repmat(Image.Or_d, numel(n_all), 1, numel(Source.tau));
...
% Calculate the SH coefficients of the observed signal if the SH
% coefficients of the directivity patterns are all one, i.e., the values of
% Image.sym_SH_coeff_cell are all one
% size(im_Obv_sym_SH_coeff_ind) = [numel(n_all), Image.num_of_images, numel(Source.tau)]
% size(im_Obv_sym_SH_coeff_ind{1}) = [(N+1)^2, numel(time_vec)]
...
im_Obv_sym_SH_coeff_ind = arrayfun(@(x, y, z, p) higher_order_source_on_z_axis_delayed(Receiver.R, x, y, z, N, time_vec, c, p),...
                                im_Or_d_mat, n_all_mat, m_all_mat, tau_mat, 'UniformOutput', false);

% Apply weights
Image.Obv_sym_SH_coeff = cellfun(@(x, y) x.*y, Image.sym_SH_coeff_cell, im_Obv_sym_SH_coeff_ind, 'UniformOutput', false);

%% Post rotation
% size(Wigner_D_cell_po) = [1, Image.num_of_images]
% size(Wigner_D_cell_po{1}) = [(N+1)^2, (N+1)^2]
Wigner_D_cell_po = arrayfun(@(x, y) WignerDM(N, x, y, 0), Image.Or_azim, Image.Or_elev, 'UniformOutput', false); 

% size(Wigner_D_cell_po_rep) = [numel(n_all), Image.num_of_images, numel(Source.tau)]
Wigner_D_cell_po_rep = repmat(Wigner_D_cell_po, numel(n_all), 1, numel(Source.tau));

% Perform post rotation
% size(Image.Obv_GL_SH_coeff) = [numel(n_all), Image.num_of_images, numel(Source.tau)]
% size(Image.Obv_GL_SH_coeff{1}) = [(N+1)^2, numel(time_vec)]
